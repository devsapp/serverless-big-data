...
1. 准备资源：创建 sqlServer 实例，并开启公网访问；
2. 部署应用；参数按照需要进行填写；
...
### 高级功能
#### 通过 vpc 连接 sqlServer 实例 
本应用默认使用的是公网连接 sqlServer 实例，在实际使用中需要考虑安全问题，因此建议您使用 VPC 连接 sqlServer 实例。具体参考：https://help.aliyun.com/document_detail/147916.html
#### 修改数据库存入点及存入方式
本应用默认将原始数据每条生成一个记录，并将整体 json decode 后存入 data 表中。如果您需要自定义转储方式，可以对 sink 函数 sink() 方法中的 sqlServer 语句进行修改。
