...
class Sink(object):
    ...
    def __init__(self):
        try:
            ...
            if not Schema(SQL_SERVER_SINK_CONFIG_SCHEMA, ignore_extra_keys=True).is_valid(env):
                Schema(SQL_SERVER_SINK_CONFIG_SCHEMA, ignore_extra_keys=True).validate(env)
